...
//--------Functions------------
	...
	private function indexPage($params){
        $user = $this->checkAuth();
        if($user !== false){ $this->redirect("friends"); }
        else
        {
            $flash = false;
            if($params !== false)
    ...
    private function friends($params){
        $user = $this->checkAuth();
        if($user === false){ $this->redirect("home/7"); }
        else
        {
            $userData = $this->model->getUserInfo($user);
            $ftweets = $this->model->getFollowersTweets($user);
            $flash = false;
            if(isset($params[0]))
            {
                $flashArr = array(
    ...
    private function newTweet($params){
        $user = $this->checkAuth();
        if($user === false){ $this->redirect("home/7"); }
        else{
            ...
            if(strlen($text) > 140)
            {
                $this->redirect("friends/2");
            }
            else
            {
                $this->model->postTweet($user, $text);
    ...
    private function publicPage($params){
        ...
        else
        {
            ...
            $tweets = $this->model->getPublicTweets($q);
            $this->loadPage($user, "public", array('tweets' => $tweets));
        }
    }
