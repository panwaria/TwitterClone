...
class Model{
    ...
    function __construct(){
        #$this->db = new mysqli('54.200.153.149', '638_user', '123456', 'Twitter');
        $this->db = new mysqli('localhost', 'root', '123456', 'Twitter');
    }
    ...
    {
        $query = "SELECT tweet_count, IF(tweet IS NULL, 'You have no Tweets', tweet) as tweet, followers, following ";
        $query .= "FROM (SELECT COUNT(*) AS tweet_count FROM Tweets WHERE user_id = " . $user->id . ") AS RC ";
        $query .= "LEFT JOIN (SELECT user_id, tweet FROM Tweets WHERE user_id = " . $user->id . " ORDER BY created_at DESC LIMIT 1) AS R"; 
    ...
    public function getFollowersTweets($user)
    {
        $query = "SELECT name, username, gravatar_hash, tweet, Tweets.created_at FROM Tweets JOIN (";
        ...
        $query .= ") AS Users on user_id = Users.id ORDER BY Tweets.created_at DESC LIMIT 10;";
        $res = $this->db->query($query);
        $ftweets = array();
        while($row = $res->fetch_object())
        {
            array_push($ftweets, $row);
        }
        return $ftweets;
    }  
    
    public function postTweet($user, $text){
        $r = array(
            "tweet" => $text,
            "created_at" => date( 'Y-m-d H:i:s'),
            "user_id" => $user->id
        );
        $this->insert("Tweets", $r);
    }
    
    public function getPublicTweets($q){
        if($q === false)
        {
            $query = "SELECT name, username, gravatar_hash, tweet, Tweets.created_at FROM Tweets JOIN Users ";
            $query .= "ON user_id = Users.id ORDER BY Tweets.created_at DESC LIMIT 10;";
        }
        else{
            $query = "SELECT name, username, gravatar_hash, tweet, Tweets.created_at FROM Tweets JOIN Users ";
            $query .= "ON user_id = Users.id WHERE tweet LIKE \"%" . $q ."%\" ORDER BY Tweets.created_at DESC LIMIT 10;";   
        }
        $res = $this->db->query($query);
        $tweets = array();
        while($row = $res->fetch_object())
    ...
    public function getPublicProfiles($user, $q){
        ...
        if($userRes->num_rows > 0){
	        ...
	        while($row = $userRes->fetch_assoc()){
	            $i = $row['id'];
	            $query .= "SELECT " . $i . " AS id, followers, IF(tweet IS NULL, 'This user has no tweets.', tweet) ";
	            $query .= "AS tweet, followed FROM (SELECT COUNT(*) as followers FROM Follows WHERE followee_id = " . $i . ") ";
	            $query .= "AS F LEFT JOIN (SELECT user_id, tweet FROM Tweets WHERE user_id = " . $i;
